...
| Version and date | 3.4.0 24-Apr-2026 |
| Disclosure restriction    |C1-Public |

**This document applies to the following products**
| Product name    |  Software version |
| :------------ | ---------------- |
| NORA-W36 series    |  3.4.0|
...
This release introduces the following user-visible changes for NORA-W36.
For the complete list of fixes and known limitations see the
[CHANGELOG-NORA-W36.md](https://github.com/u-blox/u-connectXpress/blob/main/NORA-W36/CHANGELOG-NORA-W36.md).

- **mDNS responder** — Advertise the module on the local network as `<hostname>.local` and publish DNS-SD services. New commands: `AT+UMDE`, `AT+UMDHN`, `AT+UMDSA`, `AT+UMDSR`, `AT+UMDSL`, `AT+UMDTRA`, `AT+UMDTRR`, `AT+UMDTRL`, `AT+UMDTRC`. See the [mDNS use cases](#mdns-use-cases) section.
- **Wi-Fi defaults hardened** — The default SSID has been removed for Station and Access Point, and the default authentication mode is now **WPA2** (previously OPEN). Security must now be configured **before** activating the AP or connecting the Station. See [Wi-Fi station connection](#quick-wi-fi-connection) and the migration note below.
- **Static IP for Wi-Fi AP** — New `AT+UWAPIP` and `AT+UWAPIPS` commands let you pin the AP to a fixed IPv4 address and DHCP pool instead of using the default. See [Wi-Fi access point WPA2](#wi-fi-access-point-wpa2).
- **Bluetooth advertisement filtering** — `AT+UBTDFD` lets you filter incoming advertisement packets, reducing host load and power consumption when scanning in busy environments.
- **Non-connectable Bluetooth advertising** — `AT+UBTADL` now supports non-connectable adv packets (beacon use cases without an implicit connection invitation).
- **Identity address for privacy peers** — The module now returns the resolved Bluetooth identity address as soon as it is available for peers using Resolvable Private Addresses (RPA), making bonding workflows with mobile phones more predictable.
- **Tightened pairing security** — The Bluetooth peripheral now sends a Security Request when reconnecting to a bonded central, the module always bonds with the highest possible security mode (regardless of the `AT+UBTBSM` minimum), and unauthenticated re-pairing attempts on existing bonds are rejected to protect against MITM attacks.
- **Larger binary socket I/O** — All binary read/write commands (`AT+USOWB`, `AT+USORB`, `AT+USORFB`) now support up to **1460 bytes** per call, reducing host round-trips for high-throughput TCP/UDP traffic.
...
The modules support point-to-point and point-to-multipoint configurations and can accommodate concurrent Bluetooth and Wi-Fi connections and for example act as a micro Access Point. The modules are delivered with u-connectXpress software which provides support for these Wi-Fi and Bluetooth features .
...
- **UART** — connect TX/RX between NORA-W36 and host
...
> **⚠ Migration note (since 3.4.0):** Security parameters must be configured **before** calling `AT+UWSC`. The default authentication is now WPA2 (was OPEN), and the default SSID has been removed. Existing scripts that called `AT+UWSC` with no prior `AT+UWSSW` against an open network will now fail — set the password first, or explicitly select OPEN mode with `AT+UWSSW=0,"",1`.
...
| Weak signal | Connection drops frequently | Move closer to the router, check antenna |
...
|u-connectXpress Features |Capability in 3.4.0 |
|:-----------------|:------------------|
...
|TCP MTU size | 1460 bytes link MTU; up to 1000 bytes per AT string call (`AT+USOWS`), 1460 bytes per AT binary call (`AT+USOWB`) |
|UDP MTU size | 1460 bytes link MTU; up to 1000 bytes per AT string call, 1460 bytes per AT binary call |
|HTTP body size | Rides on TCP (1460 bytes link MTU); up to 1000 bytes per AT string call (`AT+UHTCRPOS`, `AT+UHTCGBS`), 1000 bytes per AT binary read (`AT+UHTCGBB`). Binary POST/PUT (`AT+UHTCRPOB`, `AT+UHTCRPUB`) is streamed in successive calls up to the configured Content-Length |
|MQTT message size | Rides on TCP (1460 bytes link MTU); up to 1000 bytes per AT string call (`AT+UMQPS`), 5000 bytes per AT binary call (`AT+UMQPB`, `AT+UMQRB`) |


The table below summarises the major third-party components built into the firmware. The authoritative Software Bill of Materials is the CycloneDX `NORA-W36X-SBOM.json` shipped alongside the binary in each release archive.

|u-connectXpress software components |Versions in 3.4.0|
|:-----------------|:------------------|
|FreeRTOS kernel | 10.2.0 — [github.com/FreeRTOS/FreeRTOS-Kernel](https://github.com/FreeRTOS/FreeRTOS-Kernel) |
|lwIP TCP/IP stack | 2.2.0 — [github.com/lwip-tcpip/lwip](https://github.com/lwip-tcpip/lwip) |
|Mbed TLS | 3.6.4 — [github.com/Mbed-TLS/mbedtls](https://github.com/Mbed-TLS/mbedtls) |
|coreMQTT client | 2.1.0 — [github.com/FreeRTOS/coreMQTT](https://github.com/FreeRTOS/coreMQTT) |



|u-connectXpress MQTT client |Versions in 3.4.0|
|:----------------|:------------------|
|MQTT version | 3.1.1 |
...
|u-connectXpress Enterprise security | Capability in 3.4.0|
|:-----------------|:------------------|
...
|u-connectXpress TLS feature | TLS capability in 3.4.0|
|:-----------------|:------------------|
...
This chapter covers the fundamental concepts of event-driven programming with NORA-W36, including AT command response handling, Unsolicited Result Code (URC) event management, and timing considerations that are essential before diving into specific protocol implementations.
...
|u-connectXpress BLE default values | 3.4.0 |
|:----------------|:-----------------|
...
| 8 | Discover all Service Characteristics and look for **2A37**, which is the descriptor for the Heart Rate Measurement characteristics. | `AT+UBTGSCD=0,12,65535`                  | `+UBTGSCD:0,13,10,14,2A37` |
...
| 12 | Disconnect the SPS and Bluetooth connection |`AT+UBTDC=0` |`+UESPSDC:0` `+UEBTDC:0` |
...
| Nr| Instructions                              | AT command                        | AT event              |
|---|:-------------------------------------------|:-----------------------------------|:------------------------------|
| 1 | Check that Bluetooth Peripheral is enabled **2: Peripheral** or **3: Central and Peripheral**. If so, jump to step 6. | `AT+UBTM?` | `+UBTM:2` or `+UBTM:3` |
| 2 | Enable Bluetooth **2: Peripheral** or **3: Central and Peripheral** | `AT+UBTM=2` or `AT+UBTM=3` | `OK` |
| 3 | Store command                         | `AT&W`          | `OK` |
| 4 | Restart                               | `AT+CPWROFF`    | `OK` |
| 5 | Wait for NORA-W36 to start | | `+STARTUP` |
...
### Advertising options (since 3.4.0)

**Non-connectable advertising** — `AT+UBTADL` accepts a flag selecting connectable or non-connectable advertising. Use non-connectable mode for pure broadcast / beacon use cases where you do not want centrals to attempt a connection.

**Advertisement filter list** — `AT+UBTDFD` configures a filter that drops incoming advertisement packets matching the specified rule **before** they reach the host as `+UEBTAD` events. Use this to silence noisy environments and reduce host CPU and power.
...
> **⚠ Behavior change in 3.4.0:** Pairing and bonding are stricter to defend against MITM attacks. The module now:
> - **Always bonds at the highest available security mode**, even if `AT+UBTBSM` requests a lower minimum.
> - **Sends a Security Request from the peripheral** when reconnecting to a bonded central, encouraging the central to enable encryption immediately.
> - **Rejects unauthenticated pairing** when a bond already exists for the peer.
>
> For peers using Resolvable Private Addresses (RPA), the resolved **identity address** is now returned as soon as it is available, so subsequent operations targeting the bonded device work reliably.
...
| 8 | Disconnect Wi-Fi station                  | `AT+UWSDC`                       | `OK` |

Once the Wi-Fi station has an IP address, the module can open [TCP](#wi-fi-tcp-client) / [UDP](#wi-fi-udp-client) sockets, an [MQTT session](#mqtt-use-cases), or an [HTTP request](#http-and-https-use-cases). Data is exchanged in [string or binary mode](#string-mode).
...
| 8 | Station disconnect            |                    | `+UEWAPSDA:AAAAAAAAAAAA`  |
| 9 | Deactivate Wi-Fi Access Point           |    `AT+UWAPD`        | `OK` |

Once a station has joined the AP, the module can open [TCP](#wi-fi-tcp-client) / [UDP](#wi-fi-udp-client) sockets, an [MQTT session](#mqtt-use-cases), or an [HTTP request](#http-and-https-use-cases) towards the connected station. Data is exchanged in [string or binary mode](#string-mode).
...
**Static IP configuration (since 3.4.0):**

By default the AP uses `192.168.1.1` with a DHCP pool starting at `192.168.1.100`. Use `AT+UWAPIP` to pin the AP to a different IPv4 address, and `AT+UWAPIPS` to override the DHCP pool / lease settings. Both commands must be set **before** activating the AP with `AT+UWAPA`.

| Nr | Instructions | AT command | AT event |
|---|---|---|---|
| 1 | Set static IPv4 address, subnet mask and gateway for the AP | `AT+UWAPIP="10.0.0.1","255.255.255.0","10.0.0.1"` | `OK` |
| 2 | Set DHCP server pool start, end and lease time (seconds) | `AT+UWAPIPS="10.0.0.100","10.0.0.200",86400` | `OK` |
| 3 | (Optional) Persist across reboot | `AT&W` | `OK` |
| 4 | Activate the AP | `AT+UWAPA` | `OK` then `+UEWAPU`, `+UEWAPNU` |

Query the active configuration any time with `AT+UWAPIP?` and `AT+UWAPIPS?`.
...
| 3 | Close TCP socket             |  `AT+USOCL=0`                 | `+UESOCL:0`  |

With the TCP socket connected, send with `AT+USOWS` (string) or `AT+USOWB` (binary) and receive via the matching `AT+USORS` / `AT+USORB`. See [string vs binary modes](#string-mode).
...
| 4 | TCP connection is closed from remote side          |                   | `+UESOCL:1`  |
| 5 | Close TCP listener            |  `AT+USOCL=0`                 | `+UESOCL:0`  |

On the new handle (here `1`), send with `AT+USOWS` (string) or `AT+USOWB` (binary) and receive via `AT+USORS` / `AT+USORB`. See [string vs binary modes](#string-mode).
...
| 3 | Close UDP socket               |  `AT+USOCL=0`                 | `+UESOCL:0`  |

With the UDP socket bound, send datagrams with `AT+USOWS` (string) or `AT+USOWB` (binary). UDP is unidirectional per socket — use a separate listener socket to receive. See [string vs binary modes](#string-mode).
...
| 3 | Close UDP listener            |  `AT+USOCL=0`                 | `+UESOCL:0`  |

With the UDP listener bound, receive datagrams with `AT+USORS` (string) or `AT+USORB` (binary). See [string vs binary modes](#string-mode).
...
Once the TLS handshake completes (signalled by `+UESOC`), the socket is bidirectional: send with `AT+USOWS` / `AT+USOWB` and receive with `AT+USORS` / `AT+USORB`. See [string vs binary modes](#string-mode).
...
Once the mutual-TLS handshake completes (signalled by `+UESOC`), the socket is bidirectional: send with `AT+USOWS` / `AT+USOWB` and receive with `AT+USORS` / `AT+USORB`. See [string vs binary modes](#string-mode).
...
NORA-W36 stores X.509 certificates and private keys in a flat namespace and binds them by name into each TLS-enabled service (HTTPS, MQTT-S, secure sockets, EAP-TLS).

**Storage limits (NORA-W36 3.4.0, mbedTLS 3.6.4):**
- Maximum **8** credentials stored simultaneously (CAs + client certificates + private keys, **shared pool**).
- Maximum **15&nbsp;360 bytes** per credential (a single certificate **or** a full PEM-concatenated chain).
- Credential `<name>`: 1..32 characters.
- Private-key password (PKCS8/AES only): 1..64 characters.
- Format: PEM only (DER not supported).
- Re-uploading the same `<name>` overwrites the previous credential in full — there is no append; build the chain in the PEM you upload.
...
`AT+USECUBI` and `AT+USECLI` are the equivalent commands for **internal** credentials — these are not visible via `AT+USECL?` and are not removed by `AT+USECR`.

**Certificate Types (`<cert_type>`):**
- `0`: Root CA / CA bundle (used to verify the remote peer)
- `1`: Client certificate, or full client certificate chain (mTLS)
- `2`: Client private key

### Certificate chains

A chain is the list `leaf → intermediate(s) → root`. TLS verification follows signatures upward from the leaf until a certificate already present in the verifier's trust store is reached. Intermediate CAs exist so the root CA's private key can be kept offline.

A "credential" on NORA-W36 is **one slot**, regardless of how many certificates the PEM blob inside it contains. A 5-certificate chain still occupies one of the 8 slots. mbedTLS parses every `-----BEGIN CERTIFICATE-----` block in the uploaded PEM and links them internally as a chain, so simply concatenate the PEMs in chain order before calling `AT+USECUB`.

**Server CA bundle (`cert_type=0`)** — root and any intermediates the server might not send itself, order is not strict:
...
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----
<intermediate CA bytes>
-----END CERTIFICATE-----
```

**Client certificate chain (`cert_type=1`, mTLS)** — device leaf **first**, intermediates after, root **omitted** (the peer already has it):

```
-----BEGIN CERTIFICATE-----
<device leaf certificate bytes>
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----
...
NORA-W36 supports AES-encrypted PKCS8 private keys. Pass the password as the third parameter; the binary PEM payload follows in the `{ }` block exactly as for an unencrypted key:

```
AT+USECUB=2,"device_key","<password>"{<encrypted PKCS8 PEM>}
```

The password is at most 64 characters. AES is the only supported private-key encryption.

### TLS rules and limitations

- **Order matters in a client chain**: device leaf first, intermediates after, root omitted.
- **Server-CA bundle order is not strict**, but include every intermediate the server might not send itself.
- **Credential count is global**: the 8 slots are shared across CAs, client certificates and keys.
- **No append**: re-uploading a name overwrites the previous credential in full — build the chain in the PEM you upload.
- **No CRL (revocation) support**: revocation lists are not configured in the TLS context.
- **SNI is enforced by default** for TLS 1.2+ since the mbedTLS 3.6.4 update; servers without an SNI-matching certificate fail the handshake. Disable per-connection with `AT+USETE0=0` if needed.
- **EAP-TLS does not support SNI**.
- **Device clock must be within the certificate's validity window** — an unset clock is a common cause of `BADCERT_FUTURE` / `BADCERT_EXPIRED` errors. Use [NTP](#network-time-protocol-ntp-use-cases) to set the time before opening TLS connections.

### Troubleshooting TLS handshake failures

If a handshake fails with an `unknown CA` or peer-verification error, the server is most likely sending only its leaf certificate while NORA-W36 is missing one of the intermediates above it.

1. Inspect what the server actually sends:

   ```bash
   openssl s_client -connect <host>:<port> -showcerts </dev/null
   ```

2. If the server sends only its leaf certificate, fetch the missing intermediate(s) from the issuing CA and append them to the PEM bundle uploaded as `cert_type=0`.
3. Re-upload with `AT+USECUB=0,"<name>"{...}` (this overwrites the previous bundle) and retry the connection.

Useful checks on the device:

- `AT+USECL?` — confirm the credential is present under the expected name.
- `AT+USECD="<name>"` — verify subject, issuer, fingerprint and validity dates of the leaf certificate parsed from the PEM blob.
...
This use case connects NORA-W36 as a Wi-Fi Station and a MQTT client on port 1883. In this scenario, the module connects to Mosquitto without a certificate.
...
This use case connects NORA-W36 as a Wi-Fi Station and a MQTT client. In this scenario, the module connects to Mosquitto using certificates.
...
This use case connects NORA-W36 as a Wi-Fi Station and a MQTT client. In this scenario NORA-W36 connects to Thingstream using a CA Root certificate, Client-ID, Username and Password.
...
![mqtt](https://content.u-blox.com/sites/default/files/2024-02/mqtt.png)
Connect as a Wi-Fi Station and a MQTT client. In this scenario, NORA-W36 connects to Amazon AWS IoT Core using certificates.
...
Connect as a Wi-Fi Station and an Access Point and a MQTT client. In this scenario, NORA-W36 connects to Microsoft Azure using certificates.
...
**Important Note**: HTTP request and response bodies are exchanged through the AT interface in chunks. The string commands (`AT+UHTCRPOS`, `AT+UHTCRPUS`, `AT+UHTCGBS`) and the binary read (`AT+UHTCGBB`) accept up to **1000 bytes** per call (the limit declared by the HTTP command group). The binary write commands (`AT+UHTCRPOB`, `AT+UHTCRPUB`) are streamed: when `Content-Length` is set in the request header, multiple calls add up to that total; otherwise the bytes supplied in a single call become the `Content-Length`.
...
Multicast DNS (mDNS, RFC 6762) lets the module advertise itself on the local link as `<hostname>.local` so other devices can discover it without a central DNS server. Combined with DNS-SD (RFC 6763) services, hosts and apps can browse for the module by service type (for example, `_http._tcp`).

**Typical use cases**
- A phone / browser app finds the module on Wi-Fi by name (`nora-w36.local`) instead of needing the IPv4 address.
- A backend server discovers all `NORA-W36` devices on the LAN by browsing for a custom service type (e.g. `_ucx._tcp`).
- Headless commissioning — no DHCP reservation or IP printout needed.

## mDNS commands at a glance

| Command | Purpose |
|---|---|
| `AT+UMDE=<enabled>`   | Enable (`1`) / disable (`0`) the mDNS responder |
| `AT+UMDHN=<hostname>` | Set the local hostname (advertised as `<hostname>.local`). Defaults to the Wi-Fi hostname (`AT+UWHN`) if not set. |
| `AT+UMDSA=<service_type>,<protocol>,<port>[,<instance_name>]` | Register a DNS-SD service (returns `+UMDSA:<service_id>`) |
| `AT+UMDSR=<service_id>` | Remove a previously registered service |
| `AT+UMDSL?` | List all currently registered services |
| `AT+UMDTRA=<service_id>,<txt_key>,<txt_value>` | Add or update a TXT record on a service |
| `AT+UMDTRR=<service_id>,<txt_key>` | Remove a TXT record by key |
| `AT+UMDTRL=<service_id>` | List TXT records on a service |
| `AT+UMDTRC=<service_id>` | Clear all TXT records on a service |

The `+UEMDU` event signals that the responder is up and reachable; `+UEMDD` signals it is down (e.g. after Wi-Fi disconnect).

> Wi-Fi must be up (Station or AP) before enabling the responder. mDNS uses the active IPv4 / IPv6 address of the network interface. The `.local` suffix is appended automatically to the hostname.

## Quick start: advertise the module as `<name>.local`

| Nr | Instructions | AT command | AT event |
|----|--------------|------------|----------|
| 1 | Make sure Wi-Fi is connected (Station example) | see [Quick Wi-Fi connection](#quick-wi-fi-connection) | `+UEWLU`, `+UEWSNU` |
| 2 | Set the local hostname | `AT+UMDHN="nora-w36"` | `OK` |
| 3 | Enable the mDNS responder | `AT+UMDE=1` | `OK` |
| 4 | From any device on the LAN, ping the module by name | `ping nora-w36.local` | reply from module |

The responder keeps running across `AT+UWSC` reconnects. Disable it with `AT+UMDE=0`.

## Advertising a DNS-SD service

This advertises an HTTP service on TCP port 80 with two TXT key/value pairs.

| Nr | Instructions | AT command | AT event |
|----|--------------|------------|----------|
| 1 | Add a service: type, protocol (`0`=TCP, `1`=UDP), port, optional instance name | `AT+UMDSA="http",0,80,"NORA-W36 Sensor"` | `+UMDSA:0` `OK` |
| 2 | Add a TXT record (key, value) on service `0` | `AT+UMDTRA=0,"path","/api"` | `OK` |
| 3 | Add another TXT record (firmware version) | `AT+UMDTRA=0,"fw","3.4.0"` | `OK` |
| 4 | List the advertised services | `AT+UMDSL?` | `+UMDSL:0,"http",0,80,"NORA-W36 Sensor"` |
| 5 | List the TXT records on service `0` | `AT+UMDTRL=0` | `+UMDTRL:0,"path","/api"` `+UMDTRL:0,"fw","3.4.0"` |
| 6 | Remove a TXT record by key when needed | `AT+UMDTRR=0,"fw"` | `OK` |
| 7 | Clear all TXT records on a service | `AT+UMDTRC=0` | `OK` |
| 8 | Remove the service | `AT+UMDSR=0` | `OK` |

> Up to **3 services** can be registered simultaneously (`service_id` 0..3). The `protocol` parameter is an enumerator: `0` = TCP, `1` = UDP. If `<instance_name>` is omitted, the mDNS hostname is used.

## Verifying from a host

| Platform | Command | What you should see |
|---|---|---|
| macOS / Linux (Avahi) | `dns-sd -B _http._tcp` or `avahi-browse -t _http._tcp` | `NORA-W36 Sensor` appears |
| Windows (Bonjour)     | `dns-sd -B _http._tcp .` | same listing |
| Any                  | `ping nora-w36.local` | replies from the module's IPv4 address |

## Troubleshooting

- **No reply to `<name>.local`** — Some networks block multicast (UDP 224.0.0.251 / port 5353). Verify with the `dns-sd` / `avahi-browse` tools above. iOS and macOS often cache stale records; toggle Wi-Fi to clear.
- **Service not visible** — mDNS requires Wi-Fi link up first. Confirm `+UEWSNU` was received before `AT+UMDE=1`.
- **Hostname conflict** — If another device on the LAN already publishes the same hostname, the responder will append a suffix. Use `AT+UMDHN?` to read back the active hostname.
- **Maximum services / TXT records** — Up to 3 services can be registered at the same time (`service_id` 0..3). `ERROR` is returned when the limit is hit. Use `AT+UMDSL?` / `AT+UMDTRL=<id>` to inspect current state.
...
A full upload session looks like this on the wire (`01 05 3E` is the binary header, the rest is the PEM payload sent verbatim):

```text
AT+USECUB=0,"ca.pem"\x01\x05\x3E-----BEGIN CERTIFICATE-----
MIIDojCCAoqgAwIBAgIBADANBgkqhkiG9w0BAQsFADCBkDELMAkGA1UEBhMCR0Ix
...
def send_binary_data(serial_port, at_command: str, payload: bytes) -> None:
    """Send AT command + binary payload using the NORA-W36 3-byte header."""
...
with open("file.bin", "rb") as f:
    payload = f.read()
send_binary_data(serial_port, "AT+USOWB=0", payload)
```
...
SendBinaryData(port, "AT+USOWB=0", payload);
```
 ...
 *   send_binary_data(fd, "AT+USOWB=0", payload, payload_len);
 */
```
...
```
AT+UWSNST=<status_id>   // Query a specific status parameter
...
```
AT+USYCI?
+USYCI:"HwFault","timerEvent","3.4.0"
OK
```
...
**Important:** If crashes continue after firmware updates and power supply verification, [open a Short Range support case on the u-blox portal](https://portal.u-blox.com/s/topic/0TO2p000000Hr7eGAC/short-range) or e-mail [support@u-blox.com](mailto:support@u-blox.com) with the complete crash information output for further analysis.
...
- XMODEM-1K and baud rate up to 3000000 bps is supported.
...
| 3 |  Change baud rate to up to 3000000 bps (optional)  | `r 3000000` | |
...
| 9 | Check the version of the software | `AT+GMR`| `"v3.4.0"` (matches the firmware version printed on the official release zip) |
...
python xmodem.py COM3 NORA-W36X-SW-3.4.0-<build>.bin 115200

# C implementation  
xmodem.exe COM3 NORA-W36X-SW-3.4.0-<build>.bin 115200
```
...
# Examples:
python xmodem.py COM3 NORA-W36X-SW-3.4.0-<build>.bin
python xmodem.py COM3 NORA-W36X-SW-3.4.0-<build>.bin 115200
python xmodem.py /dev/ttyUSB0 NORA-W36X-SW-3.4.0-<build>.bin  # Linux
```
...
success = ublox_firmware_update("COM3", "NORA-W36X-SW-3.4.0-<build>.bin")

# Update firmware with custom baud rate
success = ublox_firmware_update("COM3", "NORA-W36X-SW-3.4.0-<build>.bin", 3000000)
```
...
# Examples:
xmodem.exe COM3 NORA-W36X-SW-3.4.0-<build>.bin
xmodem.exe COM3 NORA-W36X-SW-3.4.0-<build>.bin 115200
xmodem.exe COM15 NORA-W36X-SW-3.4.0-<build>.bin 3000000  # High COM port
```
