...
![NORA-B26](https://content.u-blox.com/sites/default/files/2026-01/NORA-B26.png)
...
This document describes how to set up and use u-blox short range stand-alone modules with u-connectXpress software for NORA-B26. It explains the functionality of different u-blox short range stand-alone modules and includes examples that describe how to use the software in different environments with AT commands. The document is applicable for Bluetooth® Low Energy (LE) modules.
...
This release introduces the following user-visible changes for NORA-B26.
For the complete list of fixes and known limitations see the
[CHANGELOG-NORA-B26.md](https://github.com/u-blox/u-connectXpress/blob/main/NORA-B26/CHANGELOG-NORA-B26.md).
...
- **Multi-connection legacy Bluetooth advertising** — `AT+UBTADL` accepts a `<max_connections>` parameter so the advertisement automatically restarts after each connection until the limit is reached. No more manual re-arming for multi-peripheral apps.
- **Extended Bluetooth advertising interval** — `AT+UBTAES` exposes the interval for extended advertisements (useful with coded PHY long-range adv).
- **Non-connectable extended Bluetooth advertising** — `AT+UBTADE` now supports non-connectable extended adv packets.
...
- **Channel Sounding (Experimental)** — `AT+UBTCSM` enables the **reflector** role for Bluetooth LE Channel Sounding, the BLE 6.0 distance measurement feature. Treat this as experimental — the API may change.
...
u-blox compact and powerful stand-alone Bluetooth Low Energy modules are designed for the development of Internet-of-Things (IoT) applications. NORA-B26 modules include an embedded Bluetooth stack and an application for wireless data transfer. The wireless support includes Bluetooth Low Energy 6.0 with Coded PHY.
...
Before starting with NORA-B26 configuration, ensure the following setup is complete:
...
- **UART** — connect TX/RX between NORA-B26 and host

**Recommended:**

- **Reset Circuit** — wire the reset pin for hardware reset
- **Bootloader Recovery Switches** — install SW1 and SW2 for easy factory reset and bootloader access (see NORA-B26 SIM for details)
...
**Current NORA-B26 UART Settings:**
...
**Expected Result:** Module appears as "NORA-B26" in Bluetooth device scans
...
|Bluetooth Qualification | Declaration ID: [TBD](https://qualification.bluetooth.com) QDID: [TBD](https://qualification.bluetooth.com) Qualification date: TBD NORA-B26 listing date: TBD |
|BLE Central connections |6 Peripherals connected |
|BLE Peripheral connections|2 Central connected |
...
The table below summarises the major third-party components built into the firmware. The authoritative Software Bill of Materials is the CycloneDX `NORA-B26X-SBOM.json` shipped alongside the binary in each release archive.
...
More information about the AT commands used in this use cases can be found in the [NORA-B26 AT command manual](https://www.u-blox.com/en/sho-online-documentation/nora-b26/at-manual).


# AT command programming

This chapter covers the fundamental concepts of event-driven programming with NORA-B26, including AT command response handling, Unsolicited Result Code (URC) event management, and timing considerations that are essential before diving into specific protocol implementations.

## Event-driven programming fundamentals

## Understanding the event model

NORA-B26 operates on an **event-driven architecture** where the module continuously monitors for various conditions and notifies the host application through events. This approach enables efficient, asynchronous programming patterns.
...
NORA-B26 generates several categories of events:
...
| BLE Connection | 10 seconds | 30 seconds | `AT+UBTC=<bd_addr>` |
...
**Unsolicited Result Codes (URCs)** are events generated by NORA-B26 without being prompted by a command. They provide real-time information about module status, incoming data, and connectivity changes.
...
This use case configures NORA-B26 as a Central device that sends and receives data from another NORA-B26 module operating as a Peripheral device. The communication between the two modules is facilitated using the SPS. It is also possible to connect to other devices that supports the SPS protocol.
...
This use case configures NORA-B26 module as a Peripheral device that sends and receives data from another NORA-B26 module operating as a Central device. The communication between the two modules is facilitated using the proprietary [u-blox Serial Port Service](https://content.u-blox.com/sites/default/files/u-connectXpress-LowEnergySerialPortService_ProtocolSpec_UBX-16011192.pdf). It is also possible to connect to other devices that support the SPS protocol.
...
**Multi-connection legacy advertising** — `AT+UBTADL=<max_connections>` lets the module automatically restart legacy advertising after each accepted connection, up to `<max_connections>` simultaneous links. This removes the need for the host to re-issue `AT+UBTAL` after every `+UEBTC` event when running peripheral fan-out scenarios.

**Extended advertising interval** — `AT+UBTAES` exposes the min/max advertising interval for extended advertisement sets, useful when tuning long-range coded-PHY beacons for power vs. discovery latency.

**Non-connectable extended advertising** — `AT+UBTADE` now supports the non-connectable mode for extended advertising sets.
...
| Nr| Instructions                          | AT command | AT event|
|---|---------------------------------------|-----------------------------------|---|
| 1 | Write SPS data in string format size |    `AT+USPSWS=0,"Hello from NORA-B26"` | `OK` |
...
Transparent mode (TM) allows the NORA-B26 to act as a transparent bridge, forwarding all UART data directly to a remote device without AT command processing. This works similarly to Data mode in legacy u-blox short range products.
...
When sending binary data with NORA-B26, you need to follow a specific format that tells the module exactly how many bytes to expect.
...
Let's send the text `Hello from NORA-B26` as binary data.

## Step-by-step calculation:

1. **Count characters**: `Hello from NORA-B26` = 19 characters = 19 bytes
...
def send_binary_data(serial_port, at_command: str, payload: bytes) -> None:
    """Send AT command + binary payload using the NORA-B26 3-byte header."""
...
Binary data is used in these NORA-B26 functions:
...
NORA-B26 uses a structured error code system organized by functional categories:
...
- Error 91 = `U_PORT_GATT_STATUS_INVALID_HANDLE`
- Action: Check connection handle

**Step 4: Apply Solution**
```bash
AT+UBTCL         // List connections
AT+UBTC=<correct_addr>
OK
```
...
This comprehensive error code reference provides immediate debugging support without requiring external documentation, significantly enhancing the troubleshooting experience for NORA-B26 users.
...
XMODEM is a file transfer protocol used for reliable data transmission over serial connections. NORA-B26 supports both standard XMODEM (128-byte blocks) and XMODEM-1K (1024-byte blocks) for firmware updates.
...
Complete working XMODEM implementations for NORA-B26 firmware updates are provided in the appendices:
...
- **Hardware Validated**: Tested with real NORA-B26 modules for firmware updates
...
- **NORA-B26 Integration**: Automated AT command sequence for firmware mode
- **High-Speed Support**: Up to 3Mbps baud rate for fast transfers

**Usage Examples:**
```bash
# Python implementation
python xmodem.py COM3 NORA-B26X-SW-3.4.0-<build>.bin 115200

# C implementation  
xmodem.exe COM3 NORA-B26X-SW-3.4.0-<build>.bin 115200
```

Both implementations use identical command-line interfaces and have been proven to work reliably with NORA-B26 hardware.

# Related information

## Documentation & resources

- **[NORA-B26 Product Page](https://www.u-blox.com/en/product/NORA-B26-series)**
...
- **[NORA-B26 AT Command Manual](https://github.com/u-blox/u-connectXpress/tree/main/NORA-B26)**
...
This Python XMODEM sender implementation has been tested with real NORA-B26 hardware for firmware updates.
...
# Examples:
python xmodem.py COM3 NORA-B26X-SW-3.4.0-<build>.bin
python xmodem.py COM3 NORA-B26X-SW-3.4.0-<build>.bin 115200
python xmodem.py /dev/ttyUSB0 NORA-B26X-SW-3.4.0-<build>.bin  # Linux
```
...
success = ublox_firmware_update("COM3", "NORA-B26X-SW-3.4.0-<build>.bin")

# Update firmware with custom baud rate
success = ublox_firmware_update("COM3", "NORA-B26X-SW-3.4.0-<build>.bin", 3000000)
```
...
# Examples:
xmodem.exe COM3 NORA-B26X-SW-3.4.0-<build>.bin
xmodem.exe COM3 NORA-B26X-SW-3.4.0-<build>.bin 115200
xmodem.exe COM15 NORA-B26X-SW-3.4.0-<build>.bin 3000000  # High COM port
```
