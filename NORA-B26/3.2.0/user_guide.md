...
- ☐ **UART Interface** - Connect TX/RX pins between NORA-B26 and host system
...
```
    ...
    • Interval: 100ms         • Device name            • "NORA-B26"
...
This chapter covers the fundamental concepts of AT command programming with NORA-B26, including command response handling, event management, and timing considerations that are essential before diving into specific protocol implementations.


This chapter covers the fundamental concepts of event-driven programming with NORA-B26, including AT command response handling, Unsolicited Result Code (URC) event management, and data flow fundamentals that are essential before diving into specific protocol implementations.
...
This chapter covers advanced data handling concepts, event processing patterns, and practical implementation strategies for building robust NORA-B26 applications.

## Data flow fundamentals

## Data mode overview

NORA-B26 supports multiple data handling modes, each optimized for different use cases:
...
This foundation in event-driven programming and data handling prepares you for implementing specific protocol use cases covered in subsequent chapters. Understanding these fundamentals is crucial for building robust IoT applications with NORA-B26.
...
NORA-B26 provides multiple data transmission modes optimized for different use cases and performance requirements. Understanding when to use each mode is crucial for optimal application performance and reliability.
...
| Nr| Instructions                          | AT command | AT event|
|---|---------------------------------------|-----------------------------------|---|
| 1 | Write SPS data in binary format size |    `AT+USPSWB=0\x01\x00\x13Hello from NORA-B26` | `OK` |
...
```
AT+USPSWB=0\x01\x00\x13Hello from NORA-B26
```
...
python xmodem.py COM3 NORA-B26X-SW-3.1.0-150.bin 115200

# C implementation  
xmodem.exe COM3 NORA-B26X-SW-3.1.0-150.bin 115200
```
...
### NORA-B26 specific implementation

**Firmware Update Process:**
1. **AT Command Mode**: Send `AT+USYFWUS=3000000` to enter XMODEM mode
2. **Protocol Detection**: NORA-B26 sends 'C' characters requesting CRC mode
...
This appendix provides visual troubleshooting aids, standardized code examples, and enhanced debugging techniques for optimal NORA-B26 implementation and troubleshooting.

## Visual status indicators and LED patterns

### NORA-B26 LED status indicators

**Official LED Patterns (from NORA-B26 Datasheet):**
...
This enhanced visual reference guide provides immediate troubleshooting support with visual indicators, standardized examples, and copy-paste ready command sequences for efficient NORA-B26 development and deployment.
...
# Examples:
python xmodem.py COM3 NORA-B26X-SW-3.1.0-150.bin
python xmodem.py COM3 NORA-B26X-SW-3.1.0-150.bin 115200
python xmodem.py /dev/ttyUSB0 NORA-B26X-SW-3.1.0-150.bin  # Linux
```
...
success = ublox_firmware_update("COM3", "NORA-B26X-SW-3.1.0-150.bin")

# Update firmware with custom baud rate
success = ublox_firmware_update("COM3", "NORA-B26X-SW-3.1.0-150.bin", 3000000)
```
...
# Examples:
xmodem.exe COM3 NORA-B26X-SW-3.1.0-150.bin
xmodem.exe COM3 NORA-B26X-SW-3.1.0-150.bin 115200
xmodem.exe COM15 NORA-B26X-SW-3.1.0-150.bin 3000000  # High COM port
```
